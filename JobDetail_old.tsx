...
export default function JobDetail() {
  ...
  const [visibleVerdicts, setVisibleVerdicts] = useState({ support: true, reject: true, neutral: true, downloaded: true })
  ...
  const { data: job } = useQuery(['job', id], async () => {
  ...
  const toggleVerdict = (verdict: 'support' | 'reject' | 'neutral' | 'downloaded') => {
  ...
  const pieData = stats ? [
    ...
    { name: 'Downloaded', value: stats.verdict_counts.downloaded || 0, type: 'downloaded' }
  ...
  const barData = stats?.year_data?.map((d: any) => ({
    ...
    downloaded: visibleVerdicts.downloaded ? d.downloaded || 0 : 0
  })) || []

  const handleDownloadCsv = async () => {
    try {
      const response = await api.get(`/jobs/${id}/download_csv`, { responseType: 'blob' })
      const url = window.URL.createObjectURL(new Blob([response.data]))
      const link = document.createElement('a')
      link.href = url
      link.setAttribute('download', `job_${id}_results.csv`)
  ...
  const handleCloneJob = () => {
    navigate('/dashboard', { state: { cloneJobData: job } })
  }
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold text-slate-900">
          {job?.job_type === 'download' ? 'Data Pipeline' : 'Analysis Job'} #{id}: {job?.query_term}
        </h1>
      ...
      {activeTab === 'logs' && (
        <div className="bg-slate-950 rounded-lg p-4 font-mono text-sm text-slate-300 h-[600px] overflow-auto whitespace-pre border border-slate-800 shadow-inner">
      ...
      {activeTab === 'results' && (
        <div className="space-y-6">
          ...
          {job?.status === 'completed' && stats && Object.values(stats.verdict_counts || {}).reduce((a: any, b: any) => a + (b || 0), 0) === 0 && (
            <div className="bg-white p-12 rounded-xl border border-slate-200 shadow-sm text-center flex flex-col items-center justify-center min-h-[400px]">
          ...
          {job?.status === 'completed' && stats && (Object.values(stats.verdict_counts || {}).reduce((a: any, b: any) => a + (b || 0), 0) as number) > 0 && (
            <>
                  ...
                  <div className="h-72">
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={barData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                        ...
                        {visibleVerdicts.neutral && <Bar dataKey="neutral" stackId="a" fill={COLORS.neutral} />}
                        {visibleVerdicts.reject && <Bar dataKey="reject" stackId="a" fill={COLORS.reject} />}
                        {visibleVerdicts.support && <Bar dataKey="support" stackId="a" fill={COLORS.support} />}
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
